...
- **Developer setup (per tenant):** API keys, Webhooks and Integrations are hidden by default. Tenant setting `allowDeveloperSetup` (toggle in Settings, only visible to platform admins) controls visibility. New permission `settings:developer` gates access to API access, Webhooks and Integrations pages. Platform admins are defined by `PLATFORM_ADMIN_EMAILS` (comma-separated). See [PERMISSIONS.md](docs/PERMISSIONS.md). **Follow-ups:** Audit events `developer_setup_enabled` / `developer_setup_disabled` when the flag is toggled; error message shown on the Settings toggle form when the user is not a platform admin; [PROVIDER.md](docs/PROVIDER.md) for enabling developer setup and using the Platform admin page; **Platform admin** page at `/dashboard/platform` (sidebar link only for `PLATFORM_ADMIN_EMAILS`) lists all tenants and allows turning developer setup on/off per tenant.
- **Documentation and help:** USER_GUIDE (in-app Help) updated: sidebar now lists Integrations; new **Integrations** section; Finance section notes journal entry detail page and that export includes finance data; Export section states backup JSON includes journal entries, exchange rates, fiscal periods; Permissions section expanded (views, Activity/Approvals/export, reference to full list for admins). [PERMISSIONS.md](docs/PERMISSIONS.md) added and linked from README.
- **Integration prep (QBO and others):** `Integration` table for per-tenant, per-provider config and encrypted credentials. `JournalEntry.externalId` + index for syncing journal entries to external systems. `lib/integration-credentials.ts` for encrypt/decrypt of OAuth tokens (set `INTEGRATION_ENCRYPTION_KEY` 32-byte hex). `lib/external-ids.ts` for reading/writing `entity.metadata.externalIds[provider]`. Audit event types: `integration_connected`, `integration_disconnected`, `integration_sync_completed`, `integration_sync_failed`. Dashboard **Integrations** page (placeholder) lists connections; connect flows to be added. See [QBO_INTEGRATION_READINESS.md](docs/QBO_INTEGRATION_READINESS.md).
